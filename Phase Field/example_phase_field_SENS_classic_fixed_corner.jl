...
# Pin the bottom-left corner in both x and y
bottom_left = Set(i for (i, n) in enumerate(grid.nodes)
                  if n.x[2] < 1e-10 && n.x[1] < 1e-10)
addnodeset!(grid, "bottom_left", bottom_left)

add!(ch, Dirichlet(:u, getfacetset(grid, "bottom"),       (x, t) -> 0.0, 2))  # y locked on all bottom
add!(ch, Dirichlet(:u, getnodeset(grid, "bottom_left"),   (x, t) -> 0.0, 1))  # x locked at corner only
add!(ch, Dirichlet(:u, getfacetset(grid, "top"),          (x, t) -> t,   1))  # x prescribed on top
close!(ch)
...
Base.invokelatest(run_phasefield, 0.025, 0.5;
    ...
    lc_fine           = 0.0075,
    n_steps_damage    = 1000,
    force_output_path = raw"C:\Users\Jesper\OneDrive - Danmarks Tekniske Universitet\Skrivebord\Uni\Kandidat\Speciale\Specialekursus\Phase Field\Plotting\force_disp_classic.txt",
    vtu_output_dir    = raw"C:\Users\Jesper\OneDrive - Danmarks Tekniske Universitet\Skrivebord\Uni\Kandidat\Speciale\Specialekursus\Phase Field\VTU\Classic")